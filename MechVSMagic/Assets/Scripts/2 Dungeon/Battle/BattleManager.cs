...
public class BattleManager : MonoBehaviour
{
    ...
    int skillidx;
    [SerializeField] GameObject skillSelectUI;    //스킬 선택 UI, 내 턴에 활성화
    ...
    public void Btn_SkillSelect(int idx)
    {
        ...
        if(skill == null)
        {
            Debug.LogError("Unknown Skill Idx");
            return;
        }

        Debug.Log(skill.name);

        if (currCaster.buffStat[(int)Obj.currAP] < currCaster.GetSkillCost(skill))
        {
            LogManager.instance.AddLog("not enough AP");
            return;
        }
        else if (currCaster.cooldowns[idx] > 0)
        {
            LogManager.instance.AddLog("Cooldown");
            return;
        }
    ...
    public void Btn_TargetSelect(int idx)
    {
        ...
        Skill s = SkillManager.GetSkill(currCaster.classIdx, GameManager.slotData.activeSkills[idx]);
