...
public class MetalKnight : Character
{
    ...
    public override void ActiveSkill(int idx, List<Unit> selects)
    {
        ...
        if(skill.idx == 58 && selects[0].turnDebuffs.buffs.Any(x=>x.name == "표식"))
        {
            ...
            float rate = tmp.effectRate[0] + ItemManager.GetSetData(5).Value[1];
        ...
        {
            skillBuffs.Add(new Buff(BuffType.Stat, LVL, new BuffOrder(), "", (int)Obj.CRC, 1, set.Value[0], 1, -2));
            skillBuffs.Add(new Buff(BuffType.Stat, LVL, new BuffOrder(), "", (int)Obj.CRB, 1, set.Value[0], 1, -2));
        }
        //오버파워 3세트 - 처형, 대구경 탄환 ATK, PEN 상승
        if((skill.idx == 52 || skill.idx == 71) && set.Value[1] > 0)
        {
            skillBuffs.Add(new Buff(BuffType.Stat, LVL, new BuffOrder(), "", (int)Obj.ATK, 1, set.Value[1], 1, -2));
            skillBuffs.Add(new Buff(BuffType.Stat, LVL, new BuffOrder(), "", (int)Obj.PEN, 1, set.Value[1], 1, -2));
        }
                ...
                case SkillType.CharSpecial1:
                    {
                        ...
                        float rate = skill.effectRate[0] + ItemManager.GetSetData(4).Value[0];
                        //막기 버프
    ...
    protected override void Passive_SkillHit(Skill active)
    {
        for (int i = 0; i < passiveIdxs.Length; i++)
        {
            ...
            if (s.idx == 74)
            {
                ...
                {
                    float rate = s.effectRate[0] + ItemManager.GetSetData(5).Value[1];
    ...
    public override KeyValuePair<bool, int> GetDamage(Unit caster, float dmg, int pen, int crb)
    {
         ...
         if (guardBuffList.Count > 0)
        {
            ...
            if (HasSkill(81))
            {
                ...
                if(set.Value[1] > 0)
                    turnBuffs.Add(new Buff(BuffType.Stat, LVL, new BuffOrder(this, orderIdx), tmp.name, (int)Obj.ATK, 1, set.Value[1], 0, tmp.effectTurn[0], 1, 1));
            }
        }
