...
public class ArmedFighter : Character
{
    ...
    public override void OnBattleStart(BattleManager BM)
    {
        ...
        if (set.Value[0] > 0)
        {
            turnBuffs.Add(new Buff(BuffType.AP, LVL, new BuffOrder(this, -1), set.Key, 31, 1, set.Value[0], 0, 99, 0, 1));
        }
    ...
    public override void ActiveSkill(int idx, List<Unit> selects)
    {
        ...
        if (skill.idx == 36)
        {
            ...
            float rate = tmp.effectRate[0] + ItemManager.GetSetData(1).Value[2];
    ...
    protected override void Passive_SkillCast(Skill active)
    {
        for (int j = 0; j < passiveIdxs.Length; j++)
        {
            ...
            if (skill.idx == 32)
            {
                ...
                {
                    //종합 타격 4세트 - 버프율 증가
                    float rate = skill.effectRate[0] + ItemManager.GetSetData(1).Value[1];
        ...
        if (set.Value[0] > 0)
        {
            skillBuffs.Add(new Buff(BuffType.Stat, LVL, new BuffOrder(this), "", (int)Obj.ATK, 1, set.Value[0], 0, -1));
        }
    }
